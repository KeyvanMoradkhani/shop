<template>
    <div>
        ...
        <aside id="column-left" class="col-sm-3 hidden-xs">
            ...
            <div class="form-group"  v-for="(attributeGroup ,index) in attributeGroups" :key="attributeGroup.id">
                <label> {{attributeGroup.title}} برند </label>
                <select class="form-control"  style="width: 100%;" aria-hidden="true" @change="addAttribute($event,index)">
                    <option >انتخاب کنید...</option>
                    <option  v-for="attributeValue in attributeGroup.atrribute_value"  :key="attributeValue.id" :value="attributeValue.id">{{attributeValue.title}}</option>
                </select>
            </div>
            <button type="submit" class="btn btn-danger" @click="sendFilter()" >اعمال فیلتر</button>
            ...
            <div class="product-filter">
                <div class="row">
                    ...
                    <div class="col-md-3 col-sm-2 text-right">
                        <select style="width: auto" id="input-sort" class="form-control col-sm-3" v-model="SortByPrice"
                                @change="GetProductBYSort()">
                    ...
                    <div class="col-sm-2 text-right">
                        <select id="input-limit" class="form-control" v-model="pagination" @change="GetProductBYSort()">
...
<script>
    export default {
        ...
        props: ['category'],
        mounted() {
            axios.get('/api/products/' + this.category.id).then(
                res => {
            ...
            axios.get('/api/attribute-groups/'+ this.category.id).then(
                res => {
        ...
        methods: {
            clickCallback: function (pageNum) {
                ...
                else if (this.SortByPrice == "ASC" || this.SortByPrice == "DESC") {
                    this.GetProductBYSort();
                } else {
                    axios.get('/api/products/' + this.category.id + '?page=' + pageNum).then(
                        res => {
                            this.products = res.data.products;
            ...
            GetProductBYSort: function () {
                this.products = [];
                axios.get('/api/sort-products/' + this.category.id + '/' + this.SortByPrice + '/' + this.pagination + '?page=' + this.page).then(
                    res => {
                        this.products = res.data.products;
                        console.log(this.products);
                    }
            ...
            sendFilter : function () {
                this.products = [];
                this.flag = true;
                var $attribute = JSON.stringify(this.computedAttribute);
                axios.get('/api/filter-product/' + this.category.id + '/' + this.SortByPrice + '/' + this.pagination + '/'+ $attribute + '?page=' + this.page).then(
                    res => {
                        this.products = res.data.products;
                    }
                ).catch(err => {
